...
template<
  ...
  , class INDEX 
  , int BITS
  , class LARGE_SORTER = SAIS<std::vector<uint32_t>, std::vector<uint32_t>::iterator>
  , class MED_SORTER = SAIS<std::vector<uint16_t>, std::vector<uint16_t>::iterator>
  , class SMALL_SORTER = SAIS<std::vector<uint8_t>, std::vector<uint8_t>::iterator>
>
class BWT_ISFM 
// // Use CRTP design pattern
// : public FmIndex<BWT_ISFM<SEQ, INDEX, BITS, LARGE_SORTER, MED_SORTER, SMALL_SORTER>> 
{
  ...
  public:
    ...
    BWT_ISFM (const SEQ& seq, MAPPER map, INDEX step = 1)
             : map_(map)
             , sample_rate_(step)
    {
        ...
        if (name < lms_size)
        {
            // TODO: use sais
            // lms_sa.push_back(0); //$
            // SMALL_SORTER sorter(lms, lms_sa.begin(), lms_size, name);
            for (auto i = 0; i < lms_size; i++)
                lms_sa[i] = i;

            std::sort(lms_sa.begin(), lms_sa.end(),
                [&lms, lms_size](auto a, auto b)
                {
                    while (a < lms_size && b < lms_size)
                    {
                        if (lms[a] < lms[b])
                            return true;
                        else if (lms[a] > lms[b])
